...
def test_queue_url_region():
    # Good URL
    assert utils.queue_url_region('https://sqs.us-east-1.1234567890/queue-name')

    # Unsafe URL
    with pytest.raises(AssertionError) as excinfo:
        utils.queue_url_region('http://sqs.us-east-1.1234567890/queue-name')
    assert str(excinfo.value) == 'Queue URL must have https scheme'

    # Wrong format
    with pytest.raises(AssertionError) as excinfo:
        utils.queue_url_region('https://foo.bar.us-east-1.1234567890/queue-name')
    assert str(excinfo.value) == 'Failed to extract AWS region from https://foo.bar.us-east-1.1234567890/queue-name'
